...
if __name__ == "__main__":
    ...
    image_dir = "/c1/kangsan/Painter/datasets/livecell/images/livecell_train_val_images/"
    panoptic_dir = '/c1/kangsan/Painter/datasets/livecell/pano_sem_seg/panoptic_segm_{}_with_color/'.format(args.split)
    save_path = os.path.join(args.output_dir, "coco_{}_image_panoptic_sem_seg.json".format(args.split))
    print(save_path)

    output_dict = []
    
    with open(f"/c1/kangsan/Painter/datasets/livecell/livecell_coco_{args.split}.json", 'r') as f:
        jf = json.load(f)
    
    for img in tqdm.tqdm(jf['images']):
        pair_dict = {}
        pair_dict["image_path"] = "livecell/images/livecell_train_val_images/" + img['file_name']
        pair_dict["target_path"] = f"livecell/pano_sem_seg/panoptic_segm_{args.split}_with_color/" + img['original_filename']
