...
class PairDataset(VisionDataset):
    ...
    def __getitem__(self, index: int) -> Tuple[Any, Any]:
        ...
        if self.use_two_pairs:
            ...
            if self.transforms2 is not None: # training
                current_lab.remove(name)
                if random.random() < 0.3:
                    demo_name = random.choice(current_lab)
                else:
                    demo_name = random.choice([x for x in self.t2tsimilarity[name][1:11] if x in current_lab])
                ...
                lab_candidates = [x for x in candidates if x in current_lab]
                if len(lab_candidates) == 0:
                    demo_name = random.choice(current_lab)
                else:
                    demo_name = lab_candidates[0]
