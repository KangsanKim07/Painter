...
def colorEncode(labelmap, colors, mode='RGB'):
    ...
    for label in unique(labelmap)[:-1]:
        if label <= 0:
            continue
        # note the color_index = class_index - 1
        labelmap_rgb += (labelmap == label)[:, :, np.newaxis] * \
            np.tile(np.array(colors[label], dtype=np.uint8), (labelmap.shape[0], labelmap.shape[1], 1))
...
if __name__ == '__main__':
    ...
    segm_dir = os.path.join(data_root, "datasets/VOC2012/SegmentationClassNum", args.split)
    save_dir = os.path.join(data_root, "datasets/VOC2012/annotations_with_color", args.split)
