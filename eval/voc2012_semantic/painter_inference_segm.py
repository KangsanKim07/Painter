...
def get_args_parser():
    ...
    parser.add_argument('--prompt', type=int, help='the number of demos in train set',
                        default=1)
...
if __name__ == '__main__':
    dataset_dir = "/c1/kangsan/Painter/datasets/"
    ...
    dst_dir = os.path.join('/c1/kangsan/Painter/models_inference/',
                           f"VOC2012_semseg_inference/demo{prompt}")
    ...
    img_src_dir = dataset_dir + "VOC2012/JPEGImages/validation"
    ...
    with open(os.path.join(dataset_dir, "VOC2012/similarity/val2train_top50-similarity.json"), 'r') as f:
    ...
    for data in tqdm.tqdm(data_loader_val):
        ...
        labeled = [x["image_path"].split('/')[-1][:-4] for x in json.load(open("/c1/kangsan/Painter/datasets/VOC2012/40samples.json"))]
        # if img_name[:-4] in labeled:
        #     continue
        ...
        candidates = [x for x in similarity[img_name[:-4]][:10] if x in labeled]
        # candidates += [x for x in similarity[img_name[:-4]][:1] if x in pseudo]
        if len(candidates) == 0:
            candidates = [x for x in similarity[img_name[:-4]] if x in labeled or x in pseudo]
            if len(candidates) == 0:
                import random
                random.seed(0)
                sample = random.choice(labeled)
                img2_paths = [dataset_dir + f"VOC2012/JPEGImages/training/{sample}.jpg"]
                tgt2_paths = [dataset_dir + f"VOC2012/annotations_with_color/training/{sample}.png"]
            else:
                if candidates[0] in pseudo:
                    img2_paths = [dataset_dir + f"VOC2012/JPEGImages/training/{candidates[0]}.jpg"]
                    tgt2_paths = [dataset_dir + f"VOC2012/pseudo/{candidates[0]}.png"]
                else:
                    img2_paths = [dataset_dir + f"VOC2012/JPEGImages/training/{candidates[0]}.jpg"]
                    tgt2_paths = [dataset_dir + f"VOC2012/annotations_with_color/training/{candidates[0]}.png"]
        else:
            img2_paths, tgt2_paths = [], []
            for x in candidates:
                if x in labeled:
                    img2_paths += [dataset_dir + f"VOC2012/JPEGImages/training/{x}.jpg"]
                    tgt2_paths += [dataset_dir + f"VOC2012/annotations_with_color/training/{x}.png"]
                # elif x in pseudo:
                #     img2_paths += [dataset_dir + f"VOC2012/JPEGImages/training/{x}.jpg"]
                #     tgt2_paths += [dataset_dir + f"VOC2012/pseudo/{x}.png"]
