...
def prepare_model(chkpt_dir, arch, args=None):
    ...
    print(msg)
    model_without_ddp.load_state_dict(torch.load(args.lora_path, map_location='cpu')['model'], strict=False)
    return model
...
if __name__ == '__main__':
    ...
    dst_dir = os.path.join('/c1/kangsan/Painter/models_inference',
                           "ap10k_inference_{}".format(os.path.basename(prompt).split(".")[0]))
    if args.flip_test:
    ...
    img_src_dir = dataset_dir + "ap10k/data_pair/val_256x192"
    if args.flip_test:
        img_src_dir += "_flip"

    dataset_val = DatasetTest(img_src_dir, input_size, ext_list=('*image.png',))
    ...
    with open(os.path.join(dataset_dir, "ap10k/similarity/val2train_top50-similarity.json"), 'r') as f:
    ...
    for data in tqdm.tqdm(data_loader_val):
        ...
        prompt = similarity[img_name[:-4]][0][:-6]
        img2_path = dataset_dir + "ap10k/data_pair/train_256x192_aug0/{}_image.png".format(prompt)
        tgt2_path = dataset_dir + "ap10k/data_pair/train_256x192_aug0/{}_label.png".format(prompt)
