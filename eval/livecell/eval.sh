...
WORK_DIR="/c1/kangsan/Painter/models_inference/${JOB_NAME}"

# inference
# python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS --master_port=29504 --use_env \
#  eval/livecell/painter_inference_pano_semseg.py \
#  --ckpt_path ${CKPT_PATH} --model ${MODEL} --prompt ${PROMPT} \
#  --input_size ${SIZE}

python -m torch.distributed.launch --nproc_per_node=$NUM_GPUS --master_port=29504 --use_env \
 eval/livecell/painter_inference_pano_inst.py \
 --ckpt_path ${CKPT_PATH} --model ${MODEL} --prompt ${PROMPT} \
 --input_size ${SIZE}

# postprocessing and eval
# python \
#   eval/livecell/COCOInstSegEvaluatorCustom.py \
